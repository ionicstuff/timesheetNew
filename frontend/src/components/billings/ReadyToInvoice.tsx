...
const ReadyToInvoice = () => {
  ...
  const handleGenerate = async (projectId: number) => {
    try {
      ...
      toast({ title: "Invoice generated", description: `Invoice #${res?.invoice?.invoiceNumber || invId} created.` });
    } catch (e: any) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>These projects are closed and have no invoice yet.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          {data.slice(0, 5).map((p) => (
            <div key={p.project_id} className="flex items-center justify-between p-3 border rounded-lg">
              <div>
          ...
          {data.length > 5 && (
            <p className="text-xs text-muted-foreground">Showing 5 of {data.length} ready projects</p>
          )}
        </div>
      </CardContent>
    </Card>
