...
const InviteMemberButton = () => {
  ...
  const loadTeamIds = async () => {
    try {
      const res = await fetch(`/api/users/team?includeSubordinates=true&limit=200`, { headers: { Authorization: `Bearer ${token}` } });
      if (res.ok) {
        const j = await res.json();
        const tm = Array.isArray(j?.data?.teamMembers) ? j.data.teamMembers : [];
  ...
  const searchCandidates = async (q: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/users?${qp.toString()}`, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error('Failed to search users');
      const j = await res.json();
      const list = Array.isArray(j?.data?.users) ? j.data.users : [];
      // Exclude current user and already-in-team
      const exclude = new Set<number>([...teamIds, currentUserId].filter(Boolean));
  ...
  useEffect(() => {
    if (!isOpen) return;
    const h = setTimeout(() => { void searchCandidates(search); }, 300);
    ...
    try {
      const results = await Promise.all(ids.map(async (uid) => {
        const res = await fetch(`/api/users/${uid}/hierarchy`, {
          ...
          body: JSON.stringify({ parentUserId: currentUserId, relationshipType: 'direct_report' })
        });
      ...
      if (failures.length === 0) {
        toast({ title: 'Team updated', description: `${ids.length} member(s) added to your team.` });
        ...
        const msg = failures.map(f => `#${f.uid}: ${f.message || 'failed'}`).join(', ');
        toast({ title: 'Some additions failed', description: msg, variant: 'destructive' });
      }
    } catch (e: any) {
      toast({ title: 'Failed to add members', description: e.message || 'Request failed', variant: 'destructive' });
    }
  };
          ...
          <div className="max-h-80 overflow-auto border rounded">
            {loading ? (
              ...
              candidates.map(u => {
                ...
                return (
                  <label key={u.id} className="flex items-center gap-2 p-2 border-b last:border-b-0 cursor-pointer">
                    <Checkbox checked={!!selected[u.id]} onCheckedChange={() => toggle(u.id)} />
                    <div className="flex-1">
                      <div className="text-sm font-medium">{label}</div>
                      <div className="text-xs text-muted-foreground">{u.designation || '—'}{sub}</div>
                    </div>
                  </label>
                );
              })
