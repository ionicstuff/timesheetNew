    ...
    const run = async () => {
      try {
        ...
        if (meRes.ok) {
          ...
          const code = (u?.roleMaster?.roleCode || u?.roleCode || '')
            .toString()
            .toUpperCase();
          const rname = (u?.roleMaster?.roleName || u?.roleName || '')
            .toString()
            .toLowerCase();
        ...
        const res = await fetch(`/api/clients/pending?limit=5`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!res.ok) {
          setItems([]);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>Clients pending completion</CardTitle>
            <CardDescription>
              Added by others — complete profile details
            </CardDescription>
          </div>
      ...
      <CardContent>
        {items.length === 0 ? (
          <div className="text-sm text-muted-foreground flex items-center gap-2">
            <Building2 className="h-4 w-4" /> No pending clients.
          </div>
        ) : (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {c.email || c.phone || 'Missing contact info'} · SPOCs:{' '}
                    {c.spocCount ?? 0}
                  </div>
                </div>
                <Button asChild size="sm" variant="outline">
                  <a href={`/clients/${c.id}`}>Complete</a>
                </Button>
              </div>
            ))}
          </div>
