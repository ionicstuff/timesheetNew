...
const ProjectForm = ({
  ...
  const [spocId, setSpocId] = useState<string>(
    initialData?.spocId ? String(initialData.spocId) : ''
  );
  const [managerId, setManagerId] = useState<string>(
    initialData?.managerId ? String(initialData.managerId) : ''
  );
  const [estimatedTime, setEstimatedTime] = useState<string>(
    initialData?.estimatedTime ? String(initialData.estimatedTime) : ''
  );
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const h = { Authorization: `Bearer ${token}` } as Record<
          string,
          string
        >;
  ...
  useEffect(() => {
    ...
    const loadSpocs = async () => {
      try {
        ...
        const token = localStorage.getItem('token') || '';
        const res = await fetch(`/api/projects/clients/${clientId}/spocs`, {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {colorOptions.map((option) => (
              <SelectItem key={option.value} value={option.value}>
                <div className="flex items-center">
                  <div
                    className={`h-3 w-3 rounded-full ${option.class} mr-2`}
                  ></div>
                  {option.label}
                </div>
              </SelectItem>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          >
            <SelectTrigger disabled={!!lockClientId}>
              <SelectValue
                placeholder={
                  loadingMeta ? 'Loading clients...' : 'Select client'
                }
              />
            </SelectTrigger>
            <SelectContent>
              ...
              {!lockClientId && (
                <SelectItem value={CREATE_CLIENT_VALUE}>
                  + Create new client…
                </SelectItem>
              )}
            </SelectContent>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(v) => {
              if (v === CREATE_SPOC_VALUE) {
                setOpenCreateSpoc(true);
                return;
              }
              setSpocId(v);
            }}
          >
            ...
            <SelectContent>
              ...
              {clientId && (
                <SelectItem value={CREATE_SPOC_VALUE}>
                  + Create new SPOC…
                </SelectItem>
              )}
            </SelectContent>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(v) => {
              if (v === SEARCH_MANAGER_VALUE) {
                setOpenSearchManager(true);
                return;
              }
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value={SEARCH_MANAGER_VALUE}>
                🔎 Search users…
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button
          type="submit"
          disabled={!name || !clientId || (!spocId && !initialData)}
        >
      ...
      >
        <QuickCreateClient
          onDone={async (createdId?: number) => {
            setOpenCreateClient(false);
            if (createdId) {
              // refresh clients and select the new one
              try {
                const token = localStorage.getItem('token') || '';
                const rowsRes = await fetch(`/api/projects/clients`, {
                  headers: { Authorization: `Bearer ${token}` },
                });
                ...
                const spocRes = await fetch(
                  `/api/projects/clients/${createdId}/spocs`,
                  { headers: { Authorization: `Bearer ${token}` } }
                );
      ...
      <Modal
        open={openCreateSpoc}
        onOpenChange={setOpenCreateSpoc}
        title="Create SPOC"
        size="md"
      >
        <QuickCreateSpoc
          ...
          onDone={async (createdId?: number) => {
            ...
            if (createdId && clientId) {
              try {
                const token = localStorage.getItem('token') || '';
                const spocRes = await fetch(
                  `/api/projects/clients/${clientId}/spocs`,
                  { headers: { Authorization: `Bearer ${token}` } }
                );
                const sp = spocRes.ok ? await spocRes.json() : [];
                setSpocs(Array.isArray(sp) ? sp : []);
      ...
      {/* Search manager */}
      <Modal
        open={openSearchManager}
        onOpenChange={setOpenSearchManager}
        title="Search Users"
        size="lg"
      >
        <QuickSearchUser
...
function QuickCreateSpoc({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => onDone(undefined)}>
          Cancel
        </Button>
        <Button
          disabled={disabled || !name || !email || busy}
          onClick={async () => {
            try {
              setBusy(true);
              const token = localStorage.getItem('token') || '';
              ...
              const data = await res.json().catch(() => ({}));
              if (!res.ok)
                throw new Error(data?.message || 'Failed to create SPOC');
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="flex justify-end mt-2">
          <Button size="sm" onClick={() => search(q)} disabled={!q || busy}>
            Search
          </Button>
        </div>
      </div>
