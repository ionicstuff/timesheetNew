...
const ProjectDetail = () => {
  ...
  const reloadProject = async () => {
    ...
    try {
      ...
      console.error('reloadProject failed', e);
    }
  };
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <Badge className={getStatusColor(project.status || 'active')}>
              {(project.status || 'active').charAt(0).toUpperCase() +
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>
                      ...
                      {(project.dueDate || project.end_date) ?? '—'}
                    </span>
                  </div>
                </div>
            ...
            <ProjectForm
              onSubmit={async (data: any) => {
                try {
                  ...
                  if (!res.ok)
                    throw new Error(j?.message || 'Failed to update project');
                  setEditOpen(false);
                  await reloadProject();
            ...
            <CardContent>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value={SEARCH_USER_VALUE}>
                          🔎 Search users…
                        </SelectItem>
                      </SelectContent>
                    </Select>
              ...
              >
                <QuickSearchUser
                  onPick={(u) => {
                    if (
                      !allUsers.some((x: any) => String(x.id) === String(u.id))
                    ) {
                      setAllUsers((prev) => [
                        ...prev,
                        {
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-8 w-8 rounded bg-blue-100 flex items-center justify-center">
                        <span className="text-blue-800 text-xs">
                          {String(file.file_type || file.filename || 'FILE')
                            .slice(0, 3)
                            .toUpperCase()}
                        </span>
                      </div>
                  ...
                  onChange={async (e) => {
                    try {
                      ...
                      for (let i = 0; i < filesSel.length; i++)
                        formData.append('file', filesSel[i]);
                      const res = await fetch(
                      ...
                      if (!res.ok) {
                        ...
                        throw new Error(
                          data?.message ||
                            'Failed to upload files. Allowed: PDF/DOC/DOCX/ZIP/JPG/PNG up to 10MB each.'
                        );
                      }
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="flex justify-end mt-2">
          <Button size="sm" disabled={!q || busy} onClick={() => search(q)}>
            Search
          </Button>
        </div>
      </div>
