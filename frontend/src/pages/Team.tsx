...
const Team = () => {
  ...
  const fetchTeam = useCallback(async (opts?: { page?: number; q?: string }) => {
    setLoading(true);
    setError(null);
    try {
      ...
      if ((opts?.q ?? query).trim()) qp.set("search", (opts?.q ?? query).trim());
      const res = await fetch(`/api/users/team?${qp.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data?.message || `Failed to load team (${res.status})`);
      }
  ...
  useEffect(() => {
    ...
    const onTeamUpdated = () => { fetchTeam({ page: 1, q: query }); setPage(1); };
    window.addEventListener('teamUpdated', onTeamUpdated as EventListener);
    return () => window.removeEventListener('teamUpdated', onTeamUpdated as EventListener);
  }, []);

  // Debounce search
  useEffect(() => {
    const h = setTimeout(() => { setPage(1); fetchTeam({ page: 1 }); }, 300);
    return () => clearTimeout(h);
  }, [query]);

  useEffect(() => { fetchTeam({ page }); }, [page]);

  const renderContent = () => {
    if (loading) return <div className="p-4">Loading team...</div>;
    if (error) return <div className="p-4 text-red-500">{error}</div>;
    if (!members.length) return <div className="p-4 text-muted-foreground">No team members found.</div>;
    ...
    return (
      <div className="divide-y">
        {members.map((u: ApiUser) => {
          ...
          const status = (u.isActive ? 'online' : 'offline') as 'online'|'away'|'offline';
          return (
            <TeamMemberCard
              key={u.id}
              id={u.id}
