    ...
    const run = async () => {
      try {
        ...
        if (projRes.ok) {
          ...
          const list = Array.isArray(pj) ? pj : (Array.isArray(pj?.projects) ? pj.projects : []);
          setProjects(list);
        } else setProjects([]);
        ...
        setClient(null); setProjects([]); setSpocs([]);
  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        if (!meRes.ok) { setCanManageSpocs(false); return; }
        const me = await meRes.json();
        const u = me?.user || me;
        const roleName = (u?.role || u?.roleName || u?.role_master?.roleName || u?.roleMaster?.roleName || '').toString().toLowerCase();
        const roleCode = (u?.roleCode || u?.role_master?.roleCode || u?.roleMaster?.roleCode || '').toString().toUpperCase();
        const allowed = new Set(['admin','director','account manager','project manager']);
        setCanManageSpocs(allowed.has(roleName) || ['ADM','DIR','AM','PM'].includes(roleCode));
      } catch { setCanManageSpocs(false); }
    };
    check();
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="flex items-center gap-4">
            <Avatar className="h-16 w-16">
              <AvatarFallback className="bg-primary/10 text-primary text-2xl">
                {(client?.clientName || client?.name || client?.companyName || 'C').charAt(0)}
              </AvatarFallback>
            </Avatar>
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-2xl font-bold">{client?.clientName || client?.name || client?.companyName || `Client #${clientId}`}</h1>
              <Badge className={(client?.status || client?.isActive) ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}>
                {(client?.status || (client?.isActive ? 'Active' : 'Inactive')) + ''}
              </Badge>
            </div>
            <p className="text-muted-foreground">{client?.industry || client?.companyName || '—'}</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <CreateTaskButton
            label="Add Task"
            overrideProjects={projects.map((p:any) => ({ id: p.id, projectName: p.projectName || p.name || p.project_name || `Project #${p.id}` }))}
          />
        </div>
      </div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="border rounded-lg p-4">
                  ...
                  <div className="mt-2 text-2xl font-bold">{Array.isArray(projects) ? projects.length : 0}</div>
                </div>
                ...
                <div className="border rounded-lg p-4">
                  ...
                  <div className="mt-2 text-2xl font-bold">{client?.pendingTasks ?? 0}</div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>{client?.contact || client?.primaryContact || '—'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm">{Array.isArray(projects) ? projects.length : 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Active Tasks</span>
                      <span className="text-sm">{client?.pendingTasks ?? 0}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Completed Tasks</span>
                      <span className="text-sm">{client?.completedTasks ?? 0}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {projects.map((project: any) => (
                  <div key={project.id} className="border rounded-lg p-4 hover:bg-muted/50">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium">{project.name || project.project_name}</h3>
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-sm text-muted-foreground">Due {project.endDate || project.end_date || '—'}</span>
                        </div>
                      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {canManageSpocs && (
                    <Button variant="outline" size="sm" onClick={() => setAddSpocOpen(true)}>
                      <Plus className="h-4 w-4 mr-2" />
                      Add Spoc
                    </Button>
                  )}
                ...
                {spocs.map((s: any) => (
                  <div key={s.id} className="flex items-center justify-between p-2 hover:bg-muted rounded">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-xs text-muted-foreground hidden sm:block">{s.designation || s.department || ''}</div>
                      ...
                      <Button size="sm" variant="ghost" onClick={() => { setSpocToDeleteId(s.id); setConfirmDeleteOpen(true); }}>Delete</Button>
                    </div>
                  </div>
                ))}
                {spocs.length === 0 && (
                  <div className="text-sm text-muted-foreground">No contacts added.</div>
                )}
              </div>
            </CardContent>
          </Card>
      ...
      {/* Edit Client Modal */}
      <Modal open={editOpen} onOpenChange={setEditOpen} title="Edit Client" size="lg">
        <ClientForm
          onSubmit={async (data: any) => {
            try {
              if (!clientId) return;
              const token = localStorage.getItem('token') || '';
              const statusNorm = (data.status ? String(data.status) : (client?.status || 'active')).toLowerCase().replace(/\s+/g, '_');
              const payload: any = {
              ...
              if (!res.ok) throw new Error(j?.message || 'Failed to update client');
              setEditOpen(false);
              // Refresh client
              try {
                const r = await fetch(`/api/clients/${clientId}`, { headers: { Authorization: `Bearer ${token}` } });
                const cj = await r.json().catch(()=>null);
                setClient(cj?.data ?? cj ?? null);
              } catch {}
              toast({ title: 'Client updated' });
            } catch (e: any) {
              toast({ title: 'Error', description: e.message || 'Failed to update client', variant: 'destructive' });
            }
          }}
      ...
      <Modal open={createProjectOpen} onOpenChange={setCreateProjectOpen} title="Add Project" size="xl">
        <ProjectForm
          ...
          onSubmit={async (data: any) => {
            try {
              ...
              const j = await res.json().catch(()=>({}));
              if (!res.ok) throw new Error(j?.message || 'Failed to create project');
              setCreateProjectOpen(false);
              // Refresh projects
              try {
                const projRes = await fetch(`/api/projects?clientId=${clientId}`, { headers: { Authorization: `Bearer ${token}` } });
      ...
      {/* Add SPOC Modal */}
      <Modal open={addSpocOpen} onOpenChange={(v)=>{ setAddSpocOpen(v); if(!v){ setEditingSpocId(null); } }} title={editingSpocId ? "Edit SPOC" : "Add SPOC"} size="md">
        <div className="space-y-4">
          <div>
            <Label>Name</Label>
            <Input value={spocName} onChange={(e)=>setSpocName(e.target.value)} placeholder="Alice" />
          </div>
          <div>
            <Label>Email</Label>
            <Input type="email" value={spocEmail} onChange={(e)=>setSpocEmail(e.target.value)} placeholder="alice@example.com" />
          </div>
          <div>
            <Label>Phone</Label>
            <Input value={spocPhone} onChange={(e)=>setSpocPhone(e.target.value)} placeholder="+1-234" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label>Designation</Label>
              <Input value={spocDesignation} onChange={(e)=>setSpocDesignation(e.target.value)} placeholder="Manager" />
            </div>
            <div>
              <Label>Department</Label>
              <Input value={spocDepartment} onChange={(e)=>setSpocDepartment(e.target.value)} placeholder="Sales" />
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox id="isPrimary" checked={spocIsPrimary} onCheckedChange={(v)=>setSpocIsPrimary(!!v)} />
            <Label htmlFor="isPrimary">Primary contact</Label>
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={()=> { setAddSpocOpen(false); setEditingSpocId(null); }}>Cancel</Button>
            <Button onClick={async ()=>{
              try {
                if (!spocName || !spocEmail) { toast({ title: 'Provide name and email', variant: 'destructive' }); return; }
                const token = localStorage.getItem('token') || '';
                const url = editingSpocId ? `/api/spocs/${editingSpocId}` : `/api/spocs`;
                ...
                const j = await res.json().catch(()=>({}));
                if (!res.ok) throw new Error(j?.message || (editingSpocId ? 'Failed to update SPOC' : 'Failed to add SPOC'));
                setAddSpocOpen(false);
                setEditingSpocId(null);
                setSpocName(''); setSpocEmail(''); setSpocPhone(''); setSpocDesignation(''); setSpocDepartment(''); setSpocIsPrimary(false);
                // Refresh spocs list
                try {
                  const spRes = await fetch(`/api/spocs/client/${clientId}`, { headers: { Authorization: `Bearer ${token}` } });
                  const sj = await spRes.json().catch(()=>({}));
                  const list = Array.isArray(sj?.data) ? sj.data : (Array.isArray(sj) ? sj : []);
                  setSpocs(list);
                } catch {}
                toast({ title: editingSpocId ? 'SPOC updated' : 'SPOC added' });
              } catch (e: any) {
                toast({ title: 'Error', description: e.message || 'Operation failed', variant: 'destructive' });
              }
            }}>{editingSpocId ? 'Save Changes' : 'Add SPOC'}</Button>
          </div>
        </div>
      </Modal>

      {/* Confirm delete SPOC */}
      <Modal open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen} title="Remove SPOC" size="sm">
        <div className="space-y-4">
          <p>Are you sure you want to remove this SPOC from the client?</p>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => { setConfirmDeleteOpen(false); setSpocToDeleteId(null); }}>Cancel</Button>
            <Button variant="destructive" onClick={async () => {
              try {
                if (!spocToDeleteId) return;
                const token = localStorage.getItem('token') || '';
                const res = await fetch(`/api/spocs/${spocToDeleteId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
                const j = await res.json().catch(()=>({}));
                if (!res.ok) throw new Error(j?.message || 'Failed to remove SPOC');
                setConfirmDeleteOpen(false); setSpocToDeleteId(null);
                // Refresh SPOCs
                try {
                  const spRes = await fetch(`/api/spocs/client/${clientId}`, { headers: { Authorization: `Bearer ${token}` } });
                  const sj = await spRes.json().catch(()=>({}));
                ...
                toast({ title: 'Error', description: e.message || 'Failed to remove SPOC', variant: 'destructive' });
              }
            }}>Remove</Button>
          </div>
        </div>
