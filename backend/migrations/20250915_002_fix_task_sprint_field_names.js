...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    if (table.sprintStartDate && !table.sprint_start_date) {
      await queryInterface.renameColumn('tasks', 'sprintStartDate', 'sprint_start_date');
    } else if (!table.sprint_start_date) {
      await queryInterface.addColumn('tasks', 'sprint_start_date', { type: Sequelize.DATE, allowNull: true });
    }

    // sprint_end_date
    if (table.sprintEndDate && !table.sprint_end_date) {
      await queryInterface.renameColumn('tasks', 'sprintEndDate', 'sprint_end_date');
    } else if (!table.sprint_end_date) {
      await queryInterface.addColumn('tasks', 'sprint_end_date', { type: Sequelize.DATE, allowNull: true });
    }
  },
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    if (table.sprint_start_date && !table.sprintStartDate) {
      await queryInterface.renameColumn('tasks', 'sprint_start_date', 'sprintStartDate');
    } else if (table.sprint_start_date) {
      await queryInterface.removeColumn('tasks', 'sprint_start_date');
    }

    if (table.sprint_end_date && !table.sprintEndDate) {
      await queryInterface.renameColumn('tasks', 'sprint_end_date', 'sprintEndDate');
    } else if (table.sprint_end_date) {
