  ...
  try {
    ...
    if (finalAssignedTo && req.user?.id && finalAssignedTo !== req.user.id) {
      ...
      if (!targetUser || !targetUser.isActive) {
        return res
          .status(400)
          .json({ message: "Target user is not active or does not exist" });
      }
  ...
  try {
    ...
    try {
      if (typeof assignedTo !== "undefined" && assignedTo !== prevAssignee) {
        ...
        await notificationSvc.notifyTaskReassigned(
          task,
          prevAssignee,
          req.user?.id || null,
        );
...
const acceptTask = async (req, res) => {
  try {
    ...
    if (task.assignedTo !== req.user.id) {
      return res
        .status(403)
        .json({ message: "You can only accept tasks assigned to you" });
    }
...
const rejectTask = async (req, res) => {
  try {
    ...
    if (task.assignedTo !== req.user.id) {
      return res
        .status(403)
        .json({ message: "You can only reject tasks assigned to you" });
    }
...
const getMyTasks = async (req, res) => {
  try {
    ...
    const limit = limitParam
      ? Math.min(parseInt(limitParam, 10) || 25, 200)
      : 100;
...
const getTaskTimeLogs = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching task time logs", error: error.message });
  }
};
...
const assignTask = async (req, res) => {
  try {
    ...
    if (
      !hasPermission &&
      task.project &&
      task.project.projectManagerId === req.user.id
    )
      hasPermission = true;
...
const startTask = async (req, res) => {
  try {
    const task = await TaskTimerService.start(
      req.params.id,
      req.user,
      req.body?.note || null,
    );
...
const pauseTask = async (req, res) => {
  try {
    const task = await TaskTimerService.pause(
      req.params.id,
      req.user,
      req.body?.note || null,
    );
...
const resumeTask = async (req, res) => {
  try {
    const task = await TaskTimerService.resume(
      req.params.id,
      req.user,
      req.body?.note || null,
    );
...
const stopTask = async (req, res) => {
  try {
    const task = await TaskTimerService.stop(
      req.params.id,
      req.user,
      req.body?.note || null,
    );
...
const completeTask = async (req, res) => {
  try {
    const task = await TaskTimerService.complete(
      req.params.id,
      req.user,
      req.body?.note || null,
    );
...
const getTaskComments = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can view comments" });
    }
...
const createTaskComment = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can comment" });
    }
...
const updateTaskComment = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can update comments" });
    }
    ...
    const canEdit =
      row.userId === req.user.id || privileged.includes(role.toLowerCase());
...
const deleteTaskComment = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can delete comments" });
    }
    const row = await TaskComment.findOne({
      where: { id: commentId, taskId: id },
    });
  ...
  uploadTaskFilesMulter.any()(req, res, async (err) => {
    ...
    try {
      ...
      if (!(await isProjectMemberByTask(id, req.user.id))) {
        return res
          .status(403)
          .json({ message: "Only project members can upload files" });
      }
...
const getTaskFiles = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can view files" });
    }
...
const downloadTaskFile = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can download files" });
    }
...
const deleteTaskFile = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can delete files" });
    }
    const row = await TaskFile.findOne({ where: { id: fileId, taskId: id } });
    if (!row) return res.status(404).json({ message: "File not found" });
    const role = req.user?.roleMaster?.roleName || "";
    ...
    const canDelete =
      row.uploadedBy === req.user.id || privileged.includes(role.toLowerCase());
    if (!canDelete) return res.status(403).json({ message: "Not allowed" });
    await row.destroy();
...
const addTaskDependency = async (req, res) => {
  try {
    ...
    if (base.projectId !== dep.projectId)
      return res
        .status(400)
        .json({ message: "Tasks must be in the same project" });
...
const likeTaskComment = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can like comments" });
    }
...
const unlikeTaskComment = async (req, res) => {
  try {
    ...
    if (!(await isProjectMemberByTask(id, req.user.id))) {
      return res
        .status(403)
        .json({ message: "Only project members can unlike comments" });
    }
